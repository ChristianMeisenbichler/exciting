...
999 continue
  ...
  subroutine fruit_summary_MPI(comm)
    ...
    integer                             :: proc, nprocs, ierr
    integer, dimension(MPI_STATUS_SIZE) :: status
    integer                             :: remoteproc, i
    ...
    if (proc == 0) then 
      ! write output for proc 0
      write (stdout,*)
      write (stdout,*)
      write (stdout,*) '    Tests on proc ', 0
      call fruit_summary
      ...
      do remoteproc = 1,nprocs-1
        ...
        do i = 1, message_index - 1
          Call MPI_recv(message_array(i), MSG_LENGTH, MPI_CHARACTER, remoteproc, 0, comm, status, ierr)
        end do
        ...
        write (stdout,*) '-----------------------------------------------------------------------'
        write (stdout,*)
        write (stdout,*) '    Tests on proc ', remoteproc
        call fruit_summary
      end do
        ...
        ! send messages
        do i = 1, message_index - 1
          Call MPI_send(message_array(i), MSG_LENGTH, MPI_CHARACTER, 0, 0, comm, ierr)
        end do
